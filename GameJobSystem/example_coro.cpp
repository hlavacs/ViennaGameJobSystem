...
namespace coro {
    ...
    Coro<int> do_compute(std::allocator_arg_t, n_pmr::memory_resource* mr) {
        ...
        auto [ret1, ret2, ret3, ret4] = co_await parallel(compute(std::allocator_arg, &g_global_mem4, 1), []() { std::cout << "GGGG\n"; }, Function([]() { std::cout << "XXXXXX\n"; }), vec, vec2, compute(std::allocator_arg, &g_global_mem4, 99));  //or std::move
