	...
	void spawn(float f1, int depth, int i2 ) {
		...
		if ( depth<2 ) { //std::rand() % 100 < 50) {
			...
			for (uint32_t i = 0; i < n; i++) {
				ThreadPool::getInstance()->addJob(std::bind(&A::spawn, this, 0.1f, depth + 1, i2), 0, "spawn");
			}
		}
		else {
			ThreadPool::getInstance()->addJob(std::bind(&A::printA, this, 0.1f, depth + 1, i2), 0, "printA");
		}
...
void case2( A& theA, uint32_t loopNumber ) {
	...
	for (uint32_t i = 0; i < 1; i++) {
		ThreadPool::getInstance()->addJob( std::bind( &A::spawn, theA, 0.1f, 0, i), 0, "spawn" ); 
	}
